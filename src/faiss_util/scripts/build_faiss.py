...
import src.faiss_util.faiss_util as faiss_util
import numpy as np
...
# combined pipeline
txt_files = glob.glob("epmc_corpus/*.txt")
docs, file_refs = [], []
...
for path in txt_files:
    ...
    for chunk in chunk_text(text):
        docs.append(chunk)
        file_refs.append(chunk)  # Store actual text chunk, not file path

print(f"Loaded {len(txt_files)} TXT files → created {len(docs)} text chunks.")
...
# 6) save
faiss_util.write_index(index, "txt_index.faiss")
